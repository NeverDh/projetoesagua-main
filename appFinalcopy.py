            ...
            if not chat_queue.empty():
                ...
                try:
                    if callable(item):
                        ...
                        with lock:
                            numero = (item["Body"]["Info"]["RemoteJid"])[2:13]
                            processoIndex = auxiliar.verificarProcesso(numero)
                            if processoIndex[0] == 1:
                                auxiliar.excluirProcessoTodos(numero)
                                integrarPlanilhas()
                            auxiliar.mensagemRecebida(item)
                        chat_queue.task_done()
                        chat_queue.queue.remove(item)
                finally:
...
def verificar_condicao_do_link():
    try:
        url = "https://v5.chatpro.com.br/chatpro-5e32485e2c/api/v1/status"
